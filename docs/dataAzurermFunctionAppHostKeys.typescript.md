...
dataAzurermFunctionAppHostKeys.DataAzurermFunctionAppHostKeys.generateConfigForImport(scope: Construct, importToId: string, importFromId: string, provider?: TerraformProvider)
```
...
```typescript
public readonly eventGridExtensionKey: string;
```
